...
class Graph:
    ...
    def iterative_deepening_search(self, start, goal):
        prev_iter_visited, depth, count = [], 0, 1
        while True:
            traced_path, visited = self.iterative_deepening_search_rec(start, goal, count, depth)
            if traced_path or len(visited) == len(prev_iter_visited): return traced_path
            else: prev_iter_visited = visited; depth += 1; count += 1

    def iterative_deepening_search_rec(self, start, goal, count, limit=-1):
        print('Iteration number ('+ str(count) +')', 'Depth limit =', limit, '\n')
        ...
        else: print('No path from {} to {}\n'.format(start, goal)); return None, visited
